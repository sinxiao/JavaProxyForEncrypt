...
import com.jxust.svsh.entity.Message;

/**
 * Home object for domain model class Message.
 * @see com.jxust.svsh.dao.Message
...
public class MessageHome {
	...
	public Message merge(Message detachedInstance) {
		...
		try {
			Message result = (Message) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Message findById(java.lang.Integer id) {
		...
		try {
			Message instance = (Message) sessionFactory.getCurrentSession().get("com.jxust.svsh.dao.Message", id);
			if (instance == null) {
	...
	public List<Message> findByExample(Message instance) {
		...
		try {
			List<Message> results = (List<Message>) sessionFactory.getCurrentSession()
					.createCriteria("com.jxust.svsh.dao.Message").add(create(instance)).list();
