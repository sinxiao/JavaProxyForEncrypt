...
import com.jxust.svsh.entity.Role;

/**
 * Home object for domain model class Role.
 * @see com.jxust.svsh.dao.Role
...
public class RoleHome {
	...
	public Role findById(java.lang.Integer id) {
		...
		try {
			Role instance = (Role) sessionFactory.getCurrentSession().get("com.jxust.svsh.dao.Role", id);
			if (instance == null) {
	...
	public List<Role> findByExample(Role instance) {
		...
		try {
			List<Role> results = (List<Role>) sessionFactory.getCurrentSession()
					.createCriteria("com.jxust.svsh.dao.Role").add(create(instance)).list();
